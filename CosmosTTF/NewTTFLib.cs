...
namespace cs_ttf {
    ...
    public class TTF {
        ...
        static byte stbtt__buf_get8(ref stbtt__buf b) {
            if (b.cursor >= b.size)
                return 0;
            return b.data[b.cursor++];
        }

        static byte stbtt__buf_peek8(ref stbtt__buf b) {
            if (b.cursor >= b.size)
                return 0;
            return b.data[b.cursor];
        }
        ...
        static UInt32 stbtt__buf_get(ref stbtt__buf b, int n) {
            UInt32 v = 0;
            int i;
        ...
        static stbtt__buf stbtt__new_buf(byte[] p, int size) {
            stbtt__buf r;
            r.data = p;
            r.size = size;
            r.cursor = 0;
        ...
        static UInt32 stbtt__buf_get16(ref stbtt__buf b) {
            return stbtt__buf_get(ref b, 2);
        }

        static UInt32 stbtt__buf_get32(ref stbtt__buf b) {
            return stbtt__buf_get(ref b, 4);
        }
        ...
        static void stbtt__dict_get_ints(ref stbtt__buf b, int key, int outcount, out List<stbtt_uint32> uint32arr) {
            int i;
        ...
        static void stbtt__dict_get_int(ref stbtt__buf b, int key, out stbtt_uint32 uint32arr) {
            int i;
            stbtt__buf operands = stbtt__dict_get(b, key);
            uint32arr = 0;
        ...
        // maybe use chars idk
        static bool stbtt_tag4(stbtt_uint8* p, char c0, char c1, char c2, char c3) {
        ...
        static bool stbtt__isfont(stbtt_uint8* font) {
            ...
            if (stbtt_tag4(font, (char)'1', (char)0, (char)0, (char)0)) return true; // TrueType 1
            ...
            if (stbtt_tag4(font, (char)0, (char)1, (char)0, (char)0)) return true; // OpenType 1.0
        ...
        public static int stbtt_GetGlyphBox(stbtt_fontinfo* info, int glyph_index, int* x0, int* y0, int* x1, int* y1) {
            if (info.cff.size != 0) {
                ...
                if (x0 != null) *x0 = ttSHORT(info.data + g + 2);
                if (y0 != null) *y0 = ttSHORT(info.data + g + 4);
                if (x1 != null) *x1 = ttSHORT(info.data + g + 6);
                if (y1 != null) *y1 = ttSHORT(info.data + g + 8);
            }
            return 1;
        }
    ...
    stbtt_int32 sx, stbtt_int32 sy, stbtt_int32 scx, stbtt_int32 scy, stbtt_int32 cx, stbtt_int32 cy) {
            ...
            if (numberOfContours > 0) {
                ...
                for (i = 0; i < n; ++i) {
                    ...
                    if (next_move == i) {
                        ...
                        stbtt_setvertex(&vertices[num_vertices++], (byte)vmove.STBTT_vmove, sx, sy, 0, 0);
                        was_off = 0;
                        ...
                        if ((flags & 1) == 0) { // if it's a curve
                            if (was_off != 0) // two off-curve control points in a row means interpolate an on-curve midpoint
                                stbtt_setvertex(&vertices[num_vertices++], (byte)vmove.STBTT_vcurve, (cx + x) >> 1, (cy + y) >> 1, cx, cy);
                            cx = x;
                            cy = y;
                            was_off = 1;
                        } else {
                            if (was_off != 0)
                                stbtt_setvertex(&vertices[num_vertices++], (byte)vmove.STBTT_vcurve, x, y, cx, cy);
                            else
                                stbtt_setvertex(&vertices[num_vertices++], (byte)vmove.STBTT_vline, x, y, 0, 0);
                            was_off = 0;
                        }
                    }
                }
                ...
                while (more != 0) {
                    ...
                    if (comp_num_verts > 0) {
                        ...
                        if ((num_vertices > 0 && vertices != null)) tmp = (stbtt_vertex*)vertices;
                        stbtt_vertex* newPtr = tmp + num_vertices;
                        newPtr = comp_verts;
                        vertices = tmp;
        ...
        struct stbtt__csctx {
            ...
            public float first_x = 0, first_y = 0;
        ...
        static stbtt__csctx STBTT__CSCTX_INIT(int bounds) {
            return new stbtt__csctx() { bounds = bounds };
        }

        static void stbtt__track_vertex(stbtt__csctx* c, stbtt_int32 x, stbtt_int32 y) {
            if (x > c.max_x || c.started == 0) c.max_x = x;
            if (y > c.max_y || c.started == 0) c.max_y = y;
            if (x < c.min_x || c.started == 0) c.min_x = x;
            if (y < c.min_y || c.started == 0) c.min_y = y;
            c.started = 1;
        }
        ...
        static void stbtt__csctx_v(stbtt__csctx* c, stbtt_uint8 type, stbtt_int32 x, stbtt_int32 y, stbtt_int32 cx, stbtt_int32 cy, stbtt_int32 cx1, stbtt_int32 cy1) {
            if (c.bounds != 0) {
                ...
                if ((vmove)type == vmove.STBTT_vcubic) {
        ...
        static void stbtt__csctx_close_shape(stbtt__csctx* ctx) {
            if (ctx.first_x != ctx.x || ctx.first_y != ctx.y)
                stbtt__csctx_v(ctx, (byte)vmove.STBTT_vline, (int)ctx.first_x, (int)ctx.first_y, 0, 0, 0, 0);
        }
        ...
        static void stbtt__csctx_rmove_to(stbtt__csctx* ctx, float dx, float dy) {
            ...
            stbtt__csctx_v(ctx, (byte)vmove.STBTT_vmove, (int)ctx.x, (int)ctx.y, 0, 0, 0, 0);
        }

        static void stbtt__csctx_rline_to(stbtt__csctx* ctx, float dx, float dy) {
            ctx.x += dx;
            ctx.y += dy;
            stbtt__csctx_v(ctx, (byte)vmove.STBTT_vline, (int)ctx.x, (int)ctx.y, 0, 0, 0, 0);
        }
        ...
        static void stbtt__csctx_rccurve_to(stbtt__csctx* ctx, float dx1, float dy1, float dx2, float dy2, float dx3, float dy3) {
            ...
            ctx.x = cx2 + dx3;
            ctx.y = cy2 + dy3;
            stbtt__csctx_v(ctx, (byte)vmove.STBTT_vcubic, (int)ctx.x, (int)ctx.y, (int)cx1, (int)cy1, (int)cx2, (int)cy2);
        }
        ...
        static int STBTT__CSERR(string s) { return 0; }
        ...
        static int stbtt__GetGlyphInfoT2(stbtt_fontinfo* info, int glyph_index, int* x0, int* y0, int* x1, int* y1) {
            ...
            if (*x0 != 0) *x0 = r != 0 ? c.min_x : 0;
            if (*y0 != 0) *y0 = r != 0 ? c.min_y : 0;
            if (*x1 != 0) *x1 = r != 0 ? c.max_x : 0;
            if (*y1 != 0) *y1 = r != 0 ? c.max_y : 0;
        ...
        public static void stbtt_GetGlyphHMetrics(stbtt_fontinfo* info, int glyph_index, int* advanceWidth, int* leftSideBearing) {
            ...
            if (glyph_index < numOfLongHorMetrics) {
                if (*advanceWidth != 0) *advanceWidth = ttSHORT(info.data + info.hmtx + 4 * glyph_index);
                if (*leftSideBearing != 0) *leftSideBearing = ttSHORT(info.data + info.hmtx + 4 * glyph_index + 2);
            } else {
                if (*advanceWidth != 0) *advanceWidth = ttSHORT(info.data + info.hmtx + 4 * (numOfLongHorMetrics - 1));
                if (*leftSideBearing != 0) *leftSideBearing = ttSHORT(info.data + info.hmtx + 4 * numOfLongHorMetrics + 2 * (glyph_index - numOfLongHorMetrics));
            }
        }

        public int stbtt_GetKerningTableLength(stbtt_fontinfo* info) {
        ...
        public int stbtt_GetKerningTable(stbtt_fontinfo* info, stbtt_kerningentry* table, int table_length) {
            stbtt_uint8* data = info.data + info.kern;
            int k, length;
        ...
        public stbtt_uint8* stbtt_FindSVGDoc(stbtt_fontinfo* info, int gl) {
            int i;
            stbtt_uint8* data = info.data;
        ...
        public int stbtt_GetGlyphSVG(stbtt_fontinfo* info, int gl, char** svg) {
            stbtt_uint8* data = info.data;
            stbtt_uint8* svg_doc;
        ...
        public int stbtt_GetCodepointSVG(stbtt_fontinfo* info, int unicode_codepoint, char** svg) {
        ...
        public static void stbtt_GetGlyphBitmapBoxSubpixel(stbtt_fontinfo* font, int glyph, float scale_x, float scale_y, float shift_x, float shift_y, int* ix0, int* iy0, int* ix1, int* iy1) {
            ...
            if (stbtt_GetGlyphBox(font, glyph, &x0, &y0, &x1, &y1) == 0) {
                ...
                if (ix0 != null) *ix0 = STBTT_ifloor(x0 * scale_x + shift_x);
                if (iy0 != null) *iy0 = STBTT_ifloor(-y1 * scale_y + shift_y);
                if (ix1 != null) *ix1 = STBTT_iceil(x1 * scale_x + shift_x);
                if (iy1 != null) *iy1 = STBTT_iceil(-y0 * scale_y + shift_y);
            }
        }
        ...
        static void* stbtt__hheap_alloc(stbtt__hheap* hh, long size, void* userdata) {
        ...
        static void stbtt__rasterize_sorted_edges(stbtt__bitmap* result, stbtt__edge* e, int n, int vsubsample, int off_x, int off_y, void* userdata) {
            ...
            while (j < result.h) {
                ...
                float tmp1 = (float)result.w, tmp2 = (float)result.w + 1;
                scanline = &(tmp1);
                scanline2 = &(tmp2);
        ...
        public static bool STBTT__COMPARE(stbtt__edge* a, stbtt__edge* b) {
            return (a.y0 < b.y0);
        }
        ...
        static stbtt__point* stbtt_FlattenCurves(stbtt_vertex* vertices, int num_verts, float objspace_flatness, int** contour_lengths, int* num_contours, void* userdata) {
            ...
            for (i = 0; i < num_verts; ++i)
                if (vertices[i].type == (byte)vmove.STBTT_vmove)
                    ++n;
                ...
                for (i = 0; i < num_verts; ++i) {
                    switch (vertices[i].type) {
                        case (byte)vmove.STBTT_vmove:
                            ...
                            break;
                        case (byte)vmove.STBTT_vline:
                            x = vertices[i].x;
                            y = vertices[i].y;
                            stbtt__add_point(points, num_points++, x, y);
                            break;
                        case (byte)vmove.STBTT_vcurve:
        ...
        public byte* stbtt_GetGlyphBitmap(stbtt_fontinfo* info, float scale_x, float scale_y, int glyph, int* width, int* height, int* xoff, int* yoff) {
        ...
        static void STBTT_memset(byte* ptr, byte data, int c) {
            for (int i = 0; i < c; i++) {
                byte* ptrTemp = (ptr + i);
                *ptrTemp = data;
            }
        ...
        static int stbtt_BakeFontBitmap_internal(byte* data, int offset,  // font location (use offset=0 for plain .ttf)
        ...
        public static int STBTT_ifloor(float x) {
        ...
        public static float STBTT_pow(float x, int y) {
            return (float)Math.Pow(x, y);
        }

        public static float STBTT_pow(float x, float y) {
            return (float)Math.Pow(x, y);
        }

        public static float STBTT_fmod(float x, float y) {
            return x % y;
        }

        public static float STBTT_fabs(float x) {
            return Math.Abs(x);
        }

        public static float STBTT_cos(float x) {
            return (float)Math.Cos(x);
        }

        public static float STBTT_acos(float x) {
        ...
        public static void STBTT_free(void* u) {
            Marshal.FreeHGlobal(new IntPtr(u));
        }
        ...
        public int stbtt_PackBegin(stbtt_pack_context* spc, byte* pixels, int pw, int ph, int stride_in_bytes, int padding, void* alloc_context) {
            ...
            if (context == null || nodes == null) {
                ...
                if (nodes != null) STBTT_free(alloc_context);
                return 0;
            }
        ...
        public static void stbtt_PackEnd(stbtt_pack_context* spc) {
            STBTT_free(spc.user_allocator_context);
            STBTT_free(spc.user_allocator_context);
        }

        static int STBTT_MAX_OVERSAMPLE = 8;
        ...
        static void stbtt__h_prefilter(byte* pixels, int w, int h, int stride_in_bytes, uint kernel_width) {
            ...
            for (j = 0; j < h; ++j) {
                int i;
                uint total;
                STBTT_memset(buffer, 0, (int)kernel_width);
        ...
        static void stbtt__v_prefilter(byte* pixels, int w, int h, int stride_in_bytes, uint kernel_width) {
            byte[] buffer = new byte[STBTT_MAX_OVERSAMPLE];
        ...
        public int stbtt_PackFontRanges(stbtt_pack_context* spc, byte* fontdata, int font_index, stbtt_pack_range* ranges, int num_ranges) {
            stbtt_fontinfo info;
        ...
        public int stbtt_PackFontRange(stbtt_pack_context* spc, byte* fontdata, int font_index, float font_size,
        ...
        static int stbtt__ray_intersect_bezier(float[] orig, float[] ray, float[] q0, float[] q1, float[] q2, float[,] hits) {
        ...
        static int stbtt__compute_crossings_x(float x, float y, int nverts, stbtt_vertex* verts) {
            ...
            for (i = 0; i < nverts; ++i) {
                if (verts[i].type == (byte)vmove.STBTT_vline) {
                ...
                if (verts[i].type == (byte)vmove.STBTT_vcurve) {
                    int x0 = (int)verts[i - 1].x, y0 = (int)verts[i - 1].y;
                    int x1 = (int)verts[i].cx, y1 = (int)verts[i].cy;
                    int x2 = (int)verts[i].x, y2 = (int)verts[i].y;
                    int ax = Math.Min(x0, Math.Min(x1, x2)), ay = Math.Min(y0, Math.Min(y1, y2));
                    int by = Math.Max(y0, Math.Max(y1, y2));
                    if (y > ay && y < by && x > ax) {
                        float[] q0 = new float[2];
                        float[] q1 = new float[2];
                        float[] q2 = new float[2];
                        float[,] hits = new float[2,2];
                        q0[0] = (float)x0;
                        q0[1] = (float)y0;
                        q1[0] = (float)x1;
                        q1[1] = (float)y1;
        ...
        // @todo: Port stbtt_GetGlyphSDF/stbtt_GetCodepointSDF/stbtt_FreeSDF if needed (probably isnt anyways)
        ...
        static int stbtt_CompareUTF8toUTF16_bigendian_internal(char* s1, int len1, char* s2, int len2) {
            return len1 == stbtt__CompareUTF8toUTF16_bigendian_prefix((stbtt_uint8*)s1, len1, (stbtt_uint8*)s2, len2) ? 1 : 0;
        }

        // returns results in whatever encoding you request... but note that 2-byte encodings
        // will be BIG-ENDIAN... use stbtt_CompareUTF8toUTF16_bigendian() to compare
        public char* stbtt_GetFontNameString(stbtt_fontinfo* font, int* length, int platformID, int encodingID, int languageID, int nameID) {
                        ...
                        if (matchlen >= 0) {
                            ...
                            if (i + 1 < count && ttUSHORT(fc + loc + 12 + 6) == next_id && ttUSHORT(fc + loc + 12) == platform && ttUSHORT(fc + loc + 12 + 2) == encoding && ttUSHORT(fc + loc + 12 + 4) == language) {
                                ...
                                if (slen == 0) {
                                    ...
                                    ++matchlen;
                                    if (stbtt_CompareUTF8toUTF16_bigendian_internal((char*)(name + matchlen), nlen - matchlen, (char*)(fc + stringOffset + off), slen) != 0)
                                        return 1;
                                }
                            } else {
        ...
        static int stbtt__matches(stbtt_uint8* fc, stbtt_uint32 offset, stbtt_uint8* name, stbtt_int32 flags) {
            stbtt_int32 nlen = (stbtt_int32)Marshal.PtrToStringAnsi(new IntPtr(name)).Length;
            stbtt_uint32 nm, hd;
        ...
        static int stbtt_FindMatchingFont_internal(byte* font_collection, char* name_utf8, stbtt_int32 flags) {
            stbtt_int32 i;
            for (i = 0; ; ++i) {
                stbtt_int32 off = stbtt_GetFontOffsetForIndex(font_collection, i);
                if (off < 0) return off;
                if (stbtt__matches((stbtt_uint8*)font_collection, (uint)off, (stbtt_uint8*)name_utf8, flags) != 0)
                    return off;
            }
        }

        public static int stbtt_BakeFontBitmap(byte* data, int offset,
                                        float pixel_height, byte* pixels, int pw, int ph,
                                        int first_char, int num_chars, stbtt_bakedchar* chardata) {
            return stbtt_BakeFontBitmap_internal((byte*)data, offset, pixel_height, pixels, pw, ph, first_char, num_chars, chardata);
        }

        public static int stbtt_GetFontOffsetForIndex(byte* data, int index) {
            return stbtt_GetFontOffsetForIndex_internal((byte*)data, index);
        }

        public static int stbtt_GetNumberOfFonts(byte* data) {
            return stbtt_GetNumberOfFonts_internal((byte*)data);
        }

        public static int stbtt_InitFont(stbtt_fontinfo* info, byte* data, int offset) {
            return stbtt_InitFont_internal(info, (byte*)data, offset);
        }

        public static int stbtt_FindMatchingFont(byte* fontdata, char* name, int flags) {
            return stbtt_FindMatchingFont_internal((byte*)fontdata, (char*)name, flags);
        }

        public static int stbtt_CompareUTF8toUTF16_bigendian(char* s1, int len1, char* s2, int len2) {
